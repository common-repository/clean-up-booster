...
if ( ! is_user_logged_in() ) {
	...
	if ( ! $access_granted ) {
		...
		if ( ! function_exists( 'get_default_taxonomy_termids_clean_up_booster' ) ) {
			/**
			 * This function is get_default_taxonomy_termids_clean_up_booster.
			 */
		...
		if ( ! function_exists( 'get_parent_termids_clean_up_booster' ) ) {
			/**
			 * This function is get_parent_termids_clean_up_booster.
			 */
		...
		if ( ! function_exists( 'get_excluded_termids_clean_up_booster' ) ) {
			/**
			 * This function is get_excluded_termids_clean_up_booster.
			 */
		...
		if ( ! function_exists( 'clean_up_booster_data' ) ) {
			/**
			 * This function is clean_up_booster_data.
			 *
			 * @param string $types passes parameter as type.
			 */
			function clean_up_booster_data( $types ) {
				global $wpdb;
				$obj   = new Dbhelper_Clean_Up_Booster();
				$where = array();
				switch ( $types ) {
					case 1:
					...
					case 15:
						...
						$wpdb->query( $query_sql ); // WPCS: db call ok, no-cache ok, unprepared SQL ok.
						echo '1';
						break;
					case 16:
					...
					case 21:
						$query = $wpdb->get_results(
							$wpdb->prepare(
								...
								WHERE tt.count = %d AND t.term_id NOT IN (' . implode( ',', get_excluded_termids_clean_up_booster() ) . ')",
								0
							)
						); // WPCS: db call ok, no-cache ok.
						if ( $query ) {
		...
		if ( isset( $_REQUEST['param'] ) ) {
			$obj_dbhelper_clean_up_booster = new Dbhelper_Clean_Up_Booster();
			switch ( sanitize_text_field( wp_unslash( $_REQUEST['param'] ) ) ) { // WPCS: CSRF ok.
				case 'wizard_clean_up_booster':
					if ( wp_verify_nonce( ( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '' ), 'clean_up_booster_check_status' ) ) {
						$type             = isset( $_REQUEST['type'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['type'] ) ) : ''; // @codingStandardsIgnoreLine
						$user_admin_email = isset( $_REQUEST['id'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['id'] ) ) : '';
						update_option( 'clean-up-booster-wizard-set-up', $type );
						if ( '' === $user_admin_email ) {
							$user_admin_email = get_option( 'admin_email' );
						}
						update_option( 'clean-up-booster-admin-email', $user_admin_email );
						if ( 'opt_in' === $type ) {
							...
							$response                             = wp_safe_remote_post(
								...
								array(
									...
									'body'        => array(
										...
										'site_id' => get_option( 'cub_tech_banker_site_id' ) != '' ? get_option( 'cub_tech_banker_site_id' ) : '', // WPCS: loose comparison ok.
										'action'  => 'plugin_analysis_data',
									),
								)
							);
							if ( ! is_wp_error( $response ) ) {
								'' != $response['body'] ? update_option( 'cub_tech_banker_site_id', $response['body'] ) : ''; // WPCS: loose comparison ok.
							}
						}
					}
					break;
				case 'manual_clean_up_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'wordpress_data_manual_clean_up' ) ) {
						$types = isset( $_REQUEST['data'] ) ? array_map( 'intval', is_array( json_decode( stripslashes( html_entity_decode( $_REQUEST['data'] ) ) ) ) ? json_decode( stripslashes( html_entity_decode( $_REQUEST['data'] ) ) ) : array() ) : array(); // WPCS: input var ok, sanitization ok.
						for ( $flag = 0; $flag < count( $types ); $flag++ ) { // @codingStandardsIgnoreLine
							clean_up_booster_data( $types[ $flag ] );
						}
					}
					break;
				case 'manual_clean_up_empty_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'empty_manual_clean_up' ) ) {
						$types = isset( $_REQUEST['delete_id'] ) ? intval( $_REQUEST['delete_id'] ) : 0;
						clean_up_booster_data( $types );
					}
					break;
				case 'bulk_action_manual_clean_up_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'manual_db_bulk_action' ) ) {
						$action     = isset( $_REQUEST['table_action'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['table_action'] ) ) : '';  // @codingStandardsIgnoreLine.
						$table_name = isset( $_REQUEST['data'] ) ? array_map( 'sanitize_text_field', is_array( json_decode( stripslashes( html_entity_decode( $_REQUEST['data'] ) ) ) ) ? json_decode( stripslashes( html_entity_decode( $_REQUEST['data'] ) ) ) : array() ) : array(); // WPCS: sanitization ok.
						if ( isset( $table_name ) && count( $table_name ) > 0 ) {
							...
							$wpdb->query(
								"SET FOREIGN_KEY_CHECKS = 1" // @codingStandardsIgnoreLine.
							); // WPCS: db call ok, no-cache ok.
						}
					}
					break;
				case 'select_action_manual_clean_up_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'manual_db_select_action' ) ) {
						$action     = isset( $_REQUEST['perform_action'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['perform_action'] ) ) : '';  // @codingStandardsIgnoreLine.
						$table_name = isset( $_REQUEST['table_name'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['table_name'] ) ) : '';
						switch ( $action ) {
							case 'optimize':
								$wpdb->query(
							...
							case 'delete':
								$wpdb->query(
									"SET FOREIGN_KEY_CHECKS = 0" // @codingStandardsIgnoreLine
								); // WPCS: db call ok, no-cache ok.
								$wpdb->query(
					...
					break;
				case 'delete_db_schedule_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'delete_db_schedule_nonce' ) ) {
						$where              = array();
						$where_parent       = array();
						$where['meta_id']   = isset( $_REQUEST['delete_id'] ) ? intval( $_REQUEST['delete_id'] ) : 0;
						$where_parent['id'] = isset( $_REQUEST['delete_id'] ) ? intval( $_REQUEST['delete_id'] ) : 0;
						$obj_dbhelper_clean_up_booster->delete_command( clean_up_booster_meta(), $where );
						$obj_dbhelper_clean_up_booster->delete_command( clean_up_booster(), $where_parent );
						$cron_name = 'database_scheduler_' . $where['meta_id'];
						unschedule_events_clean_up_booster( $cron_name );
					}
					break;

				case 'delete_selected_traffic_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'traffic_delete' ) ) {
						$where_meta            = array();
						$where_parent          = array();
						$where_meta['meta_id'] = isset( $_REQUEST['confirm_id'] ) ? intval( $_REQUEST['confirm_id'] ) : 0;
						$where_parent['id']    = isset( $_REQUEST['confirm_id'] ) ? intval( $_REQUEST['confirm_id'] ) : 0;
						$obj_dbhelper_clean_up_booster->delete_command( clean_up_booster_meta(), $where_meta );
						$obj_dbhelper_clean_up_booster->delete_command( clean_up_booster(), $where_parent );
					}
					break;

				case 'delete_selected_recent_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'recent_selected_delete' ) ) {
						$where              = array();
						$where_parent       = array();
						$where['meta_id']   = isset( $_REQUEST['login_id'] ) ? intval( $_REQUEST['login_id'] ) : 0;
						$where_parent['id'] = isset( $_REQUEST['login_id'] ) ? intval( $_REQUEST['login_id'] ) : 0;
						$obj_dbhelper_clean_up_booster->delete_command( clean_up_booster_meta(), $where );
						$obj_dbhelper_clean_up_booster->delete_command( clean_up_booster(), $where_parent );
					}
					break;
				...
				case 'visitor_log_delete_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'visitor_log_delete' ) ) {
						$where_meta            = array();
						$where_parent          = array();
						$where_meta['meta_id'] = isset( $_REQUEST['confirm_id'] ) ? intval( $_REQUEST['confirm_id'] ) : 0;
						$where_parent['id']    = isset( $_REQUEST['confirm_id'] ) ? intval( $_REQUEST['confirm_id'] ) : 0;
				...
				case 'other_settings_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'clean_up_other_settings' ) ) {
						parse_str( isset( $_REQUEST['data'] ) ? base64_decode( wp_unslash( filter_input( INPUT_POST, 'data' ) ) ) : '', $other_settings_array );
						$update_clean_up_type = array();
						$where                = array();
						if ( sanitize_text_field( $other_settings_array['ux_ddl_trackback'] ) === 'enable' ) {
							$trackback = $wpdb->query(
								$wpdb->prepare(
									'UPDATE ' . $wpdb->posts . ' SET ping_status=%s',
									'open'
								)
							); // WPCS: db call ok, no-cache ok.
						} else {
						...
						if ( 'enable' === $other_settings_array['ux_ddl_Comments'] ) {
							$comments = $wpdb->query(
								$wpdb->prepare(
									'UPDATE ' . $wpdb->posts . ' SET comment_status=%s',
									'open'
								)
							); // WPCS: db call ok, no-cache ok.
						} else {
						...
						$update_clean_up_type['ip_address_fetching_method'] = sanitize_text_field( $other_settings_array['ux_ddl_fetching_method'] );
						...
						$obj_dbhelper_clean_up_booster->update_command( clean_up_booster_meta(), $update_data, $where );
					}
					break;
				case 'blocking_options_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'clean_up_block' ) ) {
						parse_str( isset( $_REQUEST['data'] ) ? base64_decode( wp_unslash( filter_input( INPUT_POST, 'data' ) ) ) : '', $blocking_option_array );
						$update_type                  = array();
						$where                        = array();
						...
						$obj_dbhelper_clean_up_booster->update_command( clean_up_booster_meta(), $update_block_data, $where );
					}
					break;
				case 'manage_ip_address_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'clean_up_manage_ip_address' ) ) {
						parse_str( isset( $_REQUEST['data'] ) ? base64_decode( wp_unslash( filter_input( INPUT_POST, 'data' ) ) ) : '', $advance_security_data );
						$ip               = isset( $_REQUEST['ip_address'] ) ? sprintf( '%u', ip2long( sanitize_text_field( $_REQUEST['ip_address'] ) ) ) : ''; // WPCS: sanitization ok.
						$get_ip           = get_ip_location_clean_up_booster( long2ip_clean_up_booster( $ip ) );
						...
						$cub_ip_address  = get_ip_address_clean_up_booster();
						$user_ip_address = '::1' === $cub_ip_address ? sprintf( '%u', ip2long( '127.0.0.1' ) ) : sprintf( '%u', ip2long( $cub_ip_address ) );
						if ( true === $ip_exists ) {
							echo 1;
							...
							$last_id                               = $obj_dbhelper_clean_up_booster->insert_command( clean_up_booster(), $insert_manage_ip_address );
							...
							$insert_manage_ip_address['date_time']   = CLEAN_UP_BOOSTER_LOCAL_TIME;
							...
							$obj_dbhelper_clean_up_booster->insert_command( clean_up_booster_meta(), $insert_data );
							...
							if ( 'permanently' !== $time_interval ) {
								...
								schedule_clean_up_booster_ip_address_and_ranges( $cron_name, $time_interval );
							}
						}
					}
					break;
				case 'delete_ip_address_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'clean_up_manage_ip_address_delete' ) ) {
						$id                 = isset( $_REQUEST['id_address'] ) ? intval( $_REQUEST['id_address'] ) : 0;  // @codingStandardsIgnoreLine.
						$where              = array();
						$where_parent       = array();
						$where['meta_id']   = $id;
						$where_parent['id'] = $id;
				...
				case 'delete_ip_range_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'clean_up_manage_ip_ranges_delete' ) ) {
						$where              = array();
						$where_parent       = array();
						$where['meta_id']   = isset( $_REQUEST['id_range'] ) ? intval( $_REQUEST['id_range'] ) : 0;
						$where_parent['id'] = isset( $_REQUEST['id_range'] ) ? intval( $_REQUEST['id_range'] ) : 0;
				...
				case 'manage_ip_ranges_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'clean_up_manage_ip_ranges' ) ) {
						parse_str( isset( $_REQUEST['data'] ) ? base64_decode( wp_unslash( filter_input( INPUT_POST, 'data' ) ) ) : '', $ip_range_data );
						$start_ip_range = isset( $_REQUEST['start_range'] ) ? sprintf( '%u', ip2long( sanitize_text_field( wp_unslash( $_REQUEST['start_range'] ) ) ) ) : '';
						$end_ip_range   = isset( $_REQUEST['end_range'] ) ? sprintf( '%u', ip2long( sanitize_text_field( wp_unslash( $_REQUEST['end_range'] ) ) ) ) : '';
						$ip_range       = $start_ip_range . ',' . $end_ip_range;
						$get_ip         = get_ip_location_clean_up_booster( long2ip_clean_up_booster( $start_ip_range ) );
						$location       = '' == $get_ip->country_name && '' == $get_ip->city ? '' : '' == $get_ip->country_name ? '' : '' == $get_ip->city ? $get_ip->country_name : $get_ip->city . ', ' . $get_ip->country_name; // WPCS:loose comparison ok.
						...
						if ( false === $ip_exists ) {
							...
							$last_id                             = $obj_dbhelper_clean_up_booster->insert_command( clean_up_booster(), $insert_manage_ip_range );
							...
							$insert_manage_ip_range['date_time']   = CLEAN_UP_BOOSTER_LOCAL_TIME;
				...
				case 'change_email_template_module':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'email_template_data' ) ) {
						$templates           = isset( $_REQUEST['data'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['data'] ) ) : '';
						...
						$email_template_data = get_clean_up_booster_unserialize_data( $templates_data );
